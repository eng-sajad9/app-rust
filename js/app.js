...
async function loadUsers() {
  ...
  Object.entries(users).forEach(([username, data]) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="small-button btn-toggle" onclick="toggleUserBackup('${username}', ${!!data.canBackup})">
          ${data.canBackup ? "إيقاف" : "سماح"}
        </button>
      </td>
      <td>
      ...
      <td>
        ...
        ? "—"
        : `<button class="small-button" style="background:#673ab7;color:white;" onclick="openSupervisorModal('${username}', this)">👑 تخصيص</button>`
      }
      </td>
      <td>
